...
We can also apply exponantiation, Modulo and floor division easily in both language. Note that in R we can write the exponantiation "^". It is used for exclusive-or (bitwise) in Python.
...
There is a lot of datatype in both language that we will discover along the course, for now we will focus only on basic data-type that are directly available without using anyother package or library,in R common data-type are Vector, Matrix, List, Dataframe.
...
As ou can see R and Python does not store vector in the same way: R is storing them by column while python does it by row. Using "order = 'F'" alows us to store vector into matrix by column.
