...
# specify the url
urlpage = 'https://genius.com/13-organise-bande-organisee-lyrics'

def get_song_contents(urlpage):
  req = urllib3.PoolManager()
  res = req.request('GET', urlpage)
  res.status
  soup = BeautifulSoup(res.data, 'html.parser')
  contents = soup.find_all(class_= 'Lyrics__Container-sc-1ynbvzw-2 jgQsqn')
  return(contents)
...
def clean_contents(contents):
  ...
  ## throwaway unicode
  string_encode = text.encode(encoding='utf-8')
  string_decode = string_encode.decode('utf-8')
  ## remove .
  clean_txt = re.sub('[.]{2,}|\\u2005',' ',string_decode)
  return(clean_txt)
  
def split_song(clean_text)
  raw_text_splited = raw_text.split('[')
  title = raw_text_splited[2]
  splited_text = raw_text_splited[3:]
  song_structure = [str(i.split(']')[1]) for i in splited_text]

song_txt = get_song_contents(urlpage)
