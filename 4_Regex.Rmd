...
# Dealing with text in a basic Webscarpping example
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 13 17:41:21 2021
...
def get_page(urlpage,html_class):
  req = urllib3.PoolManager()
  res = req.request('GET', urlpage)
  soup = BeautifulSoup(res.data, 'html.parser')
  contents = soup.find_all(class_= html_class)
  return contents 
...
def clean_contents(contents):
  ...
  cleanr = re.compile('<.*?>')
  # replace tags by . to 
  contents = re.sub(cleanr, ' ', contents)
  ## remove .
  contents = re.sub('[ ]{1,}|[.]{2,}|\\u2005',' ',contents)
  ...
  #clean_txt = soup.text
  #clean_txt = re.sub(',[ ]{1,},',',',clean_txt)
  
  return contents
  

def get_verses(clean_txt):
  clean_txt_splited = clean_txt.split('[')
  title = clean_txt_splited[2]
  splited_txt = clean_txt_splited[3:]
  verses = [[str(i.split(']')[0]),str(i.split(']')[1])] for i in splited_txt]
  return title, verses

def get_artists_words(verses,artists_words):
    
    stopwords_f = stopwords.words('french')
    for verse in verses:
        if ':' in verse[0]:
            names = verse[0].split(':')[1]
            names = re.sub(' ','',names).lower()
            words = re.sub(r'[^\w\s]|','',verse[1])
            words = words.lower().split(' ')
            # remove french stop words
            words = np.array([word for word in words if not word in stopwords_f])
            ...
            for name_ in names_list:
                if name_ in artists_words.keys():
                    artists_words[name_].extend(words)
                else:
                    artists_words.update({str(name_):words})
    
    return artists_words
        



# specify the url   
urlpage = 'https://genius.com/albums/13-organise/13-organise'
urls = get_page(urlpage, html_class = 'u-display_block')

urls_list = []
for url in urls:
    url_clean = re.search(r'(?<=href=").*?(?=">)',str(url)).group(0)
    urls_list.append(url_clean)
...
for url in urls_list:
    ...
    while is_empty:
        try:
            song_txt = get_page(url, html_class = 'Lyrics__Container-sc-1ynbvzw-2 jgQsqn')
            clean_txt = clean_contents(song_txt)
            title, verses = get_verses(clean_txt)
            artists_words.update(get_artists_words(verses,artists_words))
            is_empty = False
            print(title)
            time.sleep(15)
        except:
            print('still empty, wait 15 more secs')
            time.sleep(15)

len(artists_words.keys())

nb_words = {name:len(artists_words[name]) for name in artists_words}
sorted(nb_words.items(), key=lambda item: item[1],reverse=True)[:10]

nb_unique_words = {name:len(set(artists_words[name])) for name in artists_words}
sorted(nb_unique_words.items(), key=lambda item: item[1],reverse=True)[:10]

Counter(artists_words["jul"]).most_common(20)
