...
dt_final_table[, 'title' := .(tolower(title)),
               by = 'pmid'][,c('ML','has_data','oa') := .(ifelse(str_detect(title,'deep learning|machine learning|neural networks?'),1,0),
...
tic()
dt_final_table[,.(sum(as.numeric(oa))/.N,
                  mean(cited)), by = 'ML']
toc()
```
...
# R data.table
tic()
dt_final_table[, .(mean_ = mean(cited),sd_ = sd(cited)),by='pub_type'][order(mean_,decreasing = T)]
toc()

tic()
dt_final_table[, .(mean_ = mean(cited),sd_ = sd(cited)),by='pub_type'][order(sd_,decreasing = T)]
toc()
```
...
countries_dt = countries_dt[,.(authors = unlist(tstrsplit(authors, "<AUTHOR>"))),by= 'pmid']
toc()
countries_dt

tic()
setkey(countries_dt,pmid,authors)
countries_dt = countries_dt[authors != ''& !str_detect(authors,'<AFFILIATION>None')]
toc()
countries_dt

tic()
countries_dt[,clean_aff := .(unlist(tstrsplit(authors,'<AFFILIATION>',keep = 2)))]
...
# R data.table
tic()
countries_dt = countries_dt[,.(clean_aff=str_replace_all(clean_aff,' USA','United States'),pmid=pmid)]
countries_dt = countries_dt[,.(clean_aff=str_replace_all(clean_aff,' UK','United Kingdom'),pmid=pmid)]
countries_dt = countries_dt[,.(clean_aff=str_replace_all(clean_aff,' Korea','South Korea'),pmid=pmid)]
countries_dt = countries_dt[,.(clean_aff = tolower(clean_aff),pmid=pmid)]
toc()
countries_dt
...
countries_dt[,country := .(ifelse(any(str_detect(clean_aff,countries)),
                                     ...
                                     "")),by=c('pmid','clean_aff')]
countries_dt = countries_dt[,-'clean_aff']
toc()
countries_dt

tic()
countries_dt = countries_dt[country != '']
...
tic()
countries_pub_dt = countries_year[,.(n = length(unique(pmid))),by=c('country','year_pub')]
countries_pub_dt[order(n,decreasing = T)]
toc()
```
...
wide_countries = dcast(countries_pub,
                       year_pub~country,
                       value.var = 'n',
                       fun.aggregate = sum)

countries_to_plot = names(sort(apply(wide_countries[, -'year_pub'],
                                     FUN = sum,
                                     MARGIN = 2),
                               decreasing = T)[1:25])
...
for(i in rev(countries_to_plot)){
  fig <- fig %>% add_trace(x = ~year_pub,
                           y = as.matrix(wide_countries[,i]),
                           name = as.character(i))
}
