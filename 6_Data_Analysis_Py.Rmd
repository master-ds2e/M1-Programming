...
def clean_pubtype(str):
  return re.sub('[{]|[}]|pubType|: |[[]|[]]','',str)


clean_pubtype(covid_papers.pubtype.loc[1])

covid_papers['pubtype'] = covid_papers.pubtype.map(lambda pubtype: clean_pubtype(pubtype))
...
pubtype_cp =  pubtype_cp[['pmid','pubtype']].groupby(['pmid','pubtype']).size().to_frame('n').reset_index()
pubtype_cp_i = pubtype_cp[['pmid','pubtype']].groupby(['pmid','pubtype']).size().to_frame('n')
```
...
casted_pubtype_cp = pubtype_cp.pivot(index="pmid", columns="pubtype", values="n").fillna(0)
...
```{python,echo = T,collapse = TRUE}
# Python
covid_papers = covid_papers.set_index('pmid')

covid_papers.loc[32837036,'doi']

covid_papers.iloc[2,2]
...
Translate the results of the last chapter's exercice using pandas.
